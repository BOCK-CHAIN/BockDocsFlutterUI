...
class _EditorPageState extends State<EditorPage> {
  ...
  Future<void> _downloadDocument(DocumentFormat format) async {
    try {
      ...
      if (kIsWeb) {
        ...
        // Mobile (Android/iOS) and Desktop download using path_provider
        try {
          ...
          try {
            // For Android/iOS, try downloads directory
            directory = await getDownloadsDirectory();
            if (directory == null) {
              // Fallback to external storage directory (Android) or documents directory
              ...
              SnackBar(
                content: Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text('Document saved to: $savePath'),
                    ),
                  ],
                ),
                backgroundColor: const Color(0xFF10B981),
          ...
          // Fallback to clipboard if file save fails (only for text)
          if (format == DocumentFormat.txt) {
            try {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('Failed to save file. Content copied to clipboard: ${e.toString()}'),
  ...
  double _getPageWidth(BuildContext context) {
    ...
    final availableWidth = screenWidth - (_showOutline ? 280 : 0) - 64; // Subtract sidebar and padding
    // Responsive width: scale between min and max based on available space
    if (availableWidth < _minPageWidth) {
      return _minPageWidth;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: !_showOutline
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              IconButton(
                icon: Icon(_isSidebarExpanded ? Icons.close : Icons.menu, color: Colors.white, size: 28),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _isLoading
                    ? const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator(color: Color(0xFF7C3AED))))
                    : documents.isEmpty
                        ? Center(
                            child: Padding(
                              padding: const EdgeInsets.all(40),
